function plot_features_par(feature_matrix, classes, feature_idx, feature_names, ...
                           out_dir, class_names, num_dims, do_par)
...
% num_dims:           should be either '2' or '3' (numeric)
% do_par:             true/false indicating if loop should be run in
...
if ~exist('num_dims', 'var') || isempty(num_dims)
    ...
    if ischar(num_dims)
        num_dims = str2double(num_dims);
    end
    
    if num_dims < 2 || num_dims > 3
        error('Number of dimensions can either be 2 or 3');
    end
end

% Check do_par
if ~exist('do_par', 'var') || isempty(do_par)
    do_par = false;
else
    if ~islogical(do_par)
        error('do_par should be logical: true or false');
    end
end
...
feature_pairs = nchoosek(feature_idx, num_dims);
        ...
        switch(do_par)
            case 0
                for plot_var = 1:length(feature_pairs)
                    ...
                    fig = figure('Units', 'centimeters', 'Position', [10 10 12 12]);
            ...
            case 1
                parfor plot_var = 1:length(feature_pairs)
                    ...
                    for class_var = 1:num_classes
                        scatter(feature_matrix(classes==class_var-1, feature_pairs(plot_var,1)), ...
                                feature_matrix(classes==class_var-1, feature_pairs(plot_var,2)), ...
                                60, scheme_colour(class_var), scheme_symbol(class_var)); %#ok<*PFBNS>
                         hold on
                    end
        ...
        switch(do_par)
            case 0
                for plot_var = 1:length(feature_pairs)
                    ...
                    % Loop over number of classes
                    for class_var = 1:num_classes
                        scatter3(feature_matrix(classes==class_var-1, feature_pairs(plot_var,1)), ...
                                 feature_matrix(classes==class_var-1, feature_pairs(plot_var,2)), ...
                                 feature_matrix(classes==class_var-1, feature_pairs(plot_var,3)), ...
                                 60, scheme_colour(class_var), scheme_symbol(class_var));
                         hold on
                    end
                    ...
                    % Z axis label
                    zlabel(feature_names(feature_pairs(plot_var,2)), ...
                           'Interpreter', 'none', 'FontWeight', 'bold');

                    % Title of the figure
                    heading = [feature_names{feature_pairs(plot_var,1)}, ' vs ', ...
                               feature_names{feature_pairs(plot_var,2)}, ' vs ', ...
                               feature_names{feature_pairs(plot_var,3)}];
                    title(heading, 'Interpreter', 'none');
